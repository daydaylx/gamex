...
from app.core.types import CompareResult, Scenario
...
def _generate_action_plan(items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    matches = [
        it
        for it in items
        if it["pair_status"] == "MATCH" and it.get("schema") == "consent_rating"
    ]

    # Filter by comfort level (both >= 3)
    comfort_filtered: List[Dict[str, Any]] = []
    for m in matches:
...
def compare(
    ...
    *,
    ...
    """
    Compare two sets of responses against a template.

    This is domain logic: no IO, no FastAPI concepts, no DB access.
    """
    items: List[Dict[str, Any]] = []
    summary = {
        "counts": {"MATCH": 0, "EXPLORE": 0, "BOUNDARY": 0},
        "flags": {
            "low_comfort_high_interest": 0,
            "big_delta": 0,
            "high_risk": 0,
        ...
        for q in mod.get("questions", []):
            ...
            if schema == "consent_rating":
                ...
                if a.get("dom_status") is not None or b.get("dom_status") is not None:
                    ...
                    row["delta_interest"] = max(
                        _abs_delta(dom_ia, dom_ib) or 0,
                        _abs_delta(sub_ia, sub_ib) or 0,
                    )
                    row["delta_comfort"] = max(
                        _abs_delta(dom_ca, dom_cb) or 0,
                        _abs_delta(sub_ca, sub_cb) or 0,
                    )
                ...
                # Handle active/passive variants
                    ...
                    row["delta_interest"] = max(
                        _abs_delta(active_ia, active_ib) or 0,
                        _abs_delta(passive_ia, passive_ib) or 0,
                    )
                    row["delta_comfort"] = max(
                        _abs_delta(active_ca, active_cb) or 0,
                        _abs_delta(passive_ca, passive_cb) or 0,
                    )
                ...
                # Standard consent_rating
                    ...
                    wants_it = ["YES", "MAYBE"]
                    if (sa == "HARD_LIMIT" and sb in wants_it) or (
                        sb == "HARD_LIMIT" and sa in wants_it
                    ):
                    ...
                    if (
                        row["delta_interest"] is not None and row["delta_interest"] >= 3
                    ) or (row["delta_comfort"] is not None and row["delta_comfort"] >= 3):
                ...
                pair_status = (
                    "MATCH"
                    if (va is not None and vb is not None and row["delta_value"] <= 1)
                    else "EXPLORE"
                )
    ...
    # 2. Compare Scenarios (optional input)
        ...
        if sa or sb:
            ...
            if choice_a and choice_b:
                if choice_a == choice_b:
                    ...
                    risk_a = sa.get("risk_type") if isinstance(sa, dict) else None
                    risk_b = sb.get("risk_type") if isinstance(sb, dict) else None
                    risky_types = [
                        "active",
                        "explore",
                        "masochism",
