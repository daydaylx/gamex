...
/**
 * Validates scale_1_10 question responses
 */
function validateScale(
  ...
  if (typeof respData === "object" && respData !== null && "value" in respData) {
    ...
    if (value !== null && value !== undefined) {
      validateRange(qid, label, "value", value, 1, 10, errors, "Wert");
    }
  }
}
...
function validateConsentRating(
  ...
  if (hasDomSub) {
    ...
    const conditions = (data.active_conditions || data.passive_conditions || data.conditions || "")
      .toString()
      .trim();
...
export function validateResponses(
  ...
  for (const [qid, respData] of Object.entries(responses || {})) {
    if (
      !respData ||
      (typeof respData !== "object" && typeof respData !== "number" && typeof respData !== "string")
    ) {
      continue;
    }
